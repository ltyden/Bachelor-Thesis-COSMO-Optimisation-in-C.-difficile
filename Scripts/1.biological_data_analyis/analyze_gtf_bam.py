...
def load_bam(path):
    ...
    A BAM index is required for random-access queries (i.e. fetching coverage
    at specific genomic coordinates). Two index naming conventions are checked:

        1. filename.bam.bai  — the samtools standard, and the format the user
                               already has alongside their BAM files.
        2. filename.bai      — an alternative convention used by some tools
                               (e.g. IGV) where the .bam extension is replaced.

    If neither index file is found, the script sorts the BAM and creates a
    fresh index automatically using pysam (this takes ~65 seconds for a file
    of ~8 million reads, but only needs to happen once).
    ...
    """
    # Both naming conventions for BAM index files
    bai_path_1 = path + ".bai"                        # e.g. file.bam.bai
    bai_path_2 = path.replace(".bam", ".bai")          # e.g. file.bai

    index_found = os.path.exists(bai_path_1) or os.path.exists(bai_path_2)

    if index_found:
        # Determine which convention was found and report it
        found_path = bai_path_1 if os.path.exists(bai_path_1) else bai_path_2
        print(f"  BAM index found        : '{os.path.basename(found_path)}'")
    else:
        # No index found — sort and index from scratch
